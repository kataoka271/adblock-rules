...
def merge(rule: StructuredRule):
    if any(r.updated for r in rule.rules):
        open(rule.path, "w", encoding="utf-8").write(
            "".join(open(r.path, "r", encoding="utf-8").read() for r in rule.rules)
        )
        print("update: {}".format(rule.path))
...
def main():
    ...
    nanj_wiki = HtmlRule(
        ...
        adblock=Rule(path="dist/supplement_rules.txt"),
        domain=Rule(path="dist/DNS_rules.txt"),
        parser=parse_nanj_wiki,
    )
    adblock_rules = StructuredRule(
        rules=[r280_adblock, nanj_filter, nanj_wiki.adblock], path="dist/01_adblock_rules.txt"
    )
    domain_ag_rules = StructuredRule(rules=[r280_domain, nanj_wiki.domain], path="dist/01_domain_ag_rules.txt")
    try:
